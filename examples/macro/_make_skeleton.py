...
NOTE_FONT   = Font(italic=True, color="444444", size=8)
NOTE_FILL   = PatternFill("solid", fgColor="EFF3FB")
...
def make_sheet(name, period_col, periods, data_cols):
    ...
    # row 2 — formula/description hints
    descs = [""] + list(data_cols.values())
    for c, desc in enumerate(descs, 1):
        cell = ws.cell(row=2, column=c, value=desc)
        cell.font  = NOTE_FONT
        cell.fill  = NOTE_FILL
        cell.alignment = Alignment(wrap_text=True)
    # data rows — period column only
    for r, period in enumerate(periods, 3):
...
make_sheet("anual", "year", range(1900, 2026), {
    ...
    "inflacion":     "as-is from BCRP (annual %)",
    # chart 8 — as-is from BCRP
    "def_fiscal":    "as-is from BCRP (% PBI, deficit = positive)",
    # chart C — pending
    "rin":           "RIN miles de millones USD  [pendiente — BCRP]",
...
make_sheet("mensual", "month", months, {
    "tcn":     "as-is from BCRP (soles/USD)",
    "tcn_var": "= 100*(tcn/tcn_prev_month - 1)  depreciacion mensual %",
})

# ── bdp (1950-2025) ───────────────────────────────────────────────────────
make_sheet("bdp", "year", range(1950, 2026), {
    "bal_com_pct":  "= 100*bal_com/pbi_nominal   balanza comercial",
    "bal_ser_pct":  "= 100*bal_ser/pbi_nominal   servicios",
    "ing_prim_pct": "= 100*ing_prim/pbi_nominal  renta de factores",
    "ing_sec_pct":  "= 100*ing_sec/pbi_nominal   transferencias",
})

# ── fiscal (1970-2025) ────────────────────────────────────────────────────
make_sheet("fiscal", "year", range(1970, 2026), {
    "ingreso":    "ingresos % PBI  as-is from BCRP",
    ...
    "deuda_pub":  "deuda publica % PBI  [pendiente — BCRP]",
})

# ── pbi_pc (1960-2024, pending chart A) ──────────────────────────────────
make_sheet("pbi_pc", "year", range(1960, 2025), {
    "peru":     "LN(PBI pc USD const)  WDI: NY.GDP.PCAP.KD  [pendiente]",
    "chile":    "LN(PBI pc USD const)  [pendiente]",
    "colombia": "LN(PBI pc USD const)  [pendiente]",
    "mexico":   "LN(PBI pc USD const)  [pendiente]",
})
...
lines = [
    ...
    ("  1. Pega los datos del addin BCRP en cada hoja (columna de periodo ya esta).", False),
    ("  2. Las columnas marcadas 'as-is' se usan directamente sin transformacion.", False),
    ("  3. Las columnas con formula deben calcularse en Excel antes de usar el batch.", False),
    ...
    ("  mensual — mensual dic-1993 a mar-2026  (columna 'month', fecha)", False),
    ("  bdp     — anual 1950-2025   (balanza de pagos)", False),
    ...
    ("  pbi_pc/*  (pbi_pc) — WDI NY.GDP.PCAP.KD o Maddison Project", False),
    ("  brecha_pbi(anual)  — requiere filtro HP (lambda=100) sobre ln(pbi)", False),
    ("", False),
    ("DIFERENCIAS VS EL SCRIPT R", True),
    ("  TCN:       el grafico de 2 paneles (facet) se divide en tcn_nivel + tcn_dep.", False),
    ("  Inflacion: el grafico con eje-y recortado se reemplaza por infla_reciente (1993:2025).", False),
]
