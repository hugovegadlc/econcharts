...
- **A theme carries preferences, not invariants.** The test: *would a different value give a different house style, or broken output?* The label-overlap floor (pitch ≥ one text height) and `PERP_SLOPE_THRESHOLD` (~17°) are geometry and live in code; `spread_gap`, `perp_gap` and `leader_after_pitches` are house style and live in `format.marks`. **Whether a fan's shade is VISIBLE is an open question, and the standard is WCAG 1.4.11** (non-text contrast): parts of a graphic required to understand the content need **3:1 against adjacent colour**, and it names chart elements explicitly. Two things it settles: the floor belongs on band-vs-BACKGROUND, not band-vs-band ("lines should contrast against their background, but do not need to contrast with each other"), and it is met by **the edge OR the filled area**, either one. The blocking measurement: **3:1 is unreachable by a pale fill at ANY alpha** — fully opaque `lightblue` is 1.79:1 and `yellow` 1.25:1 against white; only `blue` (alpha 0.46), `darkgreen` (0.53) and `teal` (0.80) can comply by fill. So "raise alpha until it complies" cannot work, and the routes are: outline the fan's outer extent only (leaning on clause (e), "if there is enough of the graphical object to understand, it passes"), outline every band, restrict `shade` to the three dark colours, or accept the gap and document it. **Undecided — do not pick one unprompted.** 0.11.1 shipped a `MIN_SHADE_DELTA_E = 5.0` floor here and was reverted: delta-E 5 was reasoned from a just-noticeable difference rather than found, and an invented threshold does not belong in the file that records invariants. The gridline was also tried as an anchor and rejected by measurement — at delta-E 17.1 it out-contrasts every band at `soft`, because a 0.5pt hairline needs far more contrast than a large area. `soft`/`medium`/`strong` stay flat alphas in `fan.strengths`.
