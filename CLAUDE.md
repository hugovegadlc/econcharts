...
- **A theme carries preferences, not invariants.** The test: *would a different value give a different house style, or broken output?* The label-overlap floor (pitch ≥ one text height) and `PERP_SLOPE_THRESHOLD` (~17°) are geometry and live in code; `spread_gap`, `perp_gap` and `leader_after_pitches` are house style and live in `format.marks`. **A fan's shade strength is the same split, drawn twice**: `soft`/`medium`/`strong` are preferences and live in `fan.strengths`, but the point below which a band is not VISIBLE is geometry of perception and lives in code (`charttypes.MIN_SHADE_DELTA_E`, a CIE76 distance of 5 from the white the transparent PNG lands on — about twice the ~2.3 just-noticeable difference, the margin a projector and a lit room take back). It could not have been a floor on ALPHA: across the bbva cycle `soft` spans delta-E 3.1 (grey) to 12.4 (blue), so one number leaves the pale end invisible or drags the dark end past what `soft` means. The floor is applied to the COMPOSITE and only ever raises, so `soft` is one perceived lightness rather than one alpha and twelve results; it binds on `grey` (0.19) and `cyan` (0.15) and touches nothing else, which is why no golden image moved. The gridline was tried as an anchor and rejected by measurement — at delta-E 17.1 it is more distinct than ANY band at `soft`, because a 0.5pt hairline needs far more contrast than a large area.
